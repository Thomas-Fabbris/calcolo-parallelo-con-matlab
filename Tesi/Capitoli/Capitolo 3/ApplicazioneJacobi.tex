...
Analogamente, il calcolo della norma del residuo $\norm{\mathbf{r}^{(k)}}$, sfruttato nel test di arresto dell'algoritmo, introduce un punto di barriera, per via del quale l'esecuzione di ciascun \textit{worker} non pu\`o procedere finch\`e tutte le altre unit\`a di elaborazione non abbiano raggiunto il medesimo punto nel flusso di esecuzione del programma.
