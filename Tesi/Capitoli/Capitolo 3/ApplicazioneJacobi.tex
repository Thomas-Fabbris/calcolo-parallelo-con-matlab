...
Tutto il codice proposto di seguito \`e stato eseguito su una workstation con 48 processore Intel\textsuperscript{\textregistered} Core\textsuperscript{\texttrademark} i7-1255U\footnote{Intel Core \`e un marchio registrato da Intel Corporation o da societ\`a da essa controllate.} e \qty{128}{\giga\byte} di memoria centrale; per brevit\`a abbiamo omesso le istruzioni impiegate nella stampa dei messaggi sulla finestra dei comandi e quelle per la creazione dei grafici.
...
Per questo motivo, in problemi \textit{compute-intensive}, si preferisce utilizzare un solutore pi\`u efficiente, come il metodo del gradiente conoiugato precondizionato, e ricorrere a tecniche che, in definitiva, diminuiscono il numero di iterazioni necessarie. \newline
