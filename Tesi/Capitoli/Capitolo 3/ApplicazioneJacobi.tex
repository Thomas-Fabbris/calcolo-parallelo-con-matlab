...
Tutto il codice proposto di seguito \`e stato eseguito su una \textit{workstation} del \textit{Control Systems and Automation Laboratory} (CAL) dell'Universit\`a degli Studi di Bergamo, caratterizzata da 24 processori fisici Intel\textsuperscript{\textregistered} Xeon\textsuperscript{\texttrademark} E5-2687W v4\footnote{Xeon \`e un marchio registrato da Intel Corporation o da societ\`a da essa controllate.} e \qty{125}{\giga\byte} di memoria centrale; relativamente al software impiegato, ci siamo avvalsi di un'installazione di MATLAB R2024b compilata per la versione a 64 bit del sistema operativo Linux.\newline
Per brevit\`a, abbiamo omesso le istruzioni impiegate per la stampa dei messaggi sulla finestra dei comandi e quelle per la creazione dei grafici.
...
Questa scelta ci consente di stimare facilmente l'accuratezza della soluzione calcolata dal metodo.
...
il numero massimo di iterazioni \lstinline{maxit} \`e fissato a $\num{100}$, mentre la tolleranza cercata durante la soluzione del problema \`e pari a ${10}^{-6}$.\newline
    ...
    raggiungere la tolleranza desiderata (1e-06) poiché
    ...
    \end{matlaboutput}
    \begin{matlaboutput}
    Tempo esecuzione jacobi: 451.9124 sec.
    \end{matlaboutput}
    ...
    una soluzione accettabile nel primo tentativo.

    Il tempo di esecuzione sperimentato, pari a circa otto minuti per un problema di dimensioni non strabilianti, rappresenta un segnale chiaro
    dell'intensit\`a computazionale del metodo di Jacobi e giustifica il ricorso agli strumenti del calcolo parallelo con l'auspicio di registrare un miglioramento prestazionale.
...
All'invocazione di \lstinline{distributed}, MATLAB avvia automaticamente un \textit{parallel pool} usando l'ambiente di esecuzione parallela predefinito e, dal momento che la funzione \lstinline{sum} \`e stata progettata per operare su diversi tipi di input, \lstinline{b} \`e automaticamente un \textit{array} distribuito, essendo
ottenuto come somma di altri \textit{array} distribuiti.
\begin{matlabcode}
...
\begin{matlaboutput}
    ...
    Tempo esecuzione jacobi con array distribuiti: 875.5708 sec.
\end{matlaboutput}
...
Per questo motivo, nell'affrontare problemi \textit{compute-intensive}, si preferisce utilizzare un solutore pi\`u efficiente, come il metodo del gradiente coniugato precondizionato, e ricorrere a tecniche che diminuiscono il numero di iterazioni necessarie. \newline
...
\end{equation*}
Perfino nel terzo scenario di esecuzione non abbiamo registrato alcun incremento di velocit\`a rispetto alla versione seriale dell'algoritmo, dal momento che l'\textit{overhead} per il coordinamento e la sincronizzazione dei \textit{worker} supera di gran lunga i benefici introdotti dalla parallelizzazione.

L'analisi del profilo di esecuzione del codice di \lstinline{jacobi} mediante lo strumento \textit{Profiler} di MATLAB ha immediatamente evidenziato quale possa essere il principale collo di bottiglia: la funzione interna \lstinline{RemoteSpmdControllerImpl.awaitBlock} occupa la quasi totalit\`a del tempo di esecuzione del metodo di Jacobi, indicando che i \textit{worker} sono rimasti in uno stato di attesa per un intervallo di tempo significativo e non hanno proceduto con la computazione vera e propria nel frattempo.

Innanzitutto, questo comportamento peculiare pu\'o essere spiegato dalla presenza di una latenza di comunicazione intrinseca nell'infrastruttura del Parallel Computing Toolbox; nonostante i \textit{worker} risiedano sullo stesso calcolatore, la comunicazione all'interno del \textit{parallel pool} non \`e a costo zero e l'effetto negativo sulle prestazioni si aggiunge progressivamente al ritardo accumulato in precedenza.

Inoltre, il metodo di Jacobi \`e un algoritmo fortemente sincrono; a ogni passo dell'esecuzione, impone un coordinamento tra i \textit{worker} per calcolare la nuova soluzione approssimata e per valutare le condizioni di arresto basate sul controllo del residuo relativo e sulla verifica della divergenza del metodo.\newline
L'aggiornamento del residuo del sistema $\mathbf{r}^{(k)}$, previsto da $\eqref{eq:metodoIterativoConResiduo}$, induce un'operazione di riduzione, in quanto \`e necessario recuperare l'intero vettore delle incognite $\mathbf{x}^{(k)}$ per determinare completamente il prodotto matrice-vettore $A\mathbf{x}^{(k)}$.\newline
Analogamente, il calcolo della norma del residuo relativo $\norm{\mathbf{r}^{(k)}}$, sfruttato nel test di arresto dell'algoritmo, introduce un punto di barriera, secondo il quale l'esecuzione di ciascun \textit{worker} non pu\`o procedere finch\`e tutte le altre unit\`a di elaborazione non abbiano raggiunto il medesimo punto nel flusso di esecuzione del programma.

In conclusione, il risultato ottenuto dall'applicazione della funzione \lstinline{jacobi} per la risoluzione di un sistema di equazioni lineari di grandi dimensioni si \`e rivelato negativo in termini di prestazioni, ma ha permesso di dimostrare come la scelta dell'algoritmo da eseguire influenzi la possibilit\`a di ottenere un reale \textit{speedup} delle prestazioni, mettendo in luce i limiti pratici del parallelismo a livello di dati in un sistema a memoria condivisa.
