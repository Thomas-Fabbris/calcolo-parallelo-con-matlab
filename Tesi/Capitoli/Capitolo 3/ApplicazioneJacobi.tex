...
il numero massimo di iterazioni \lstinline{maxit} \`e fissato a $\num{100}$, mentre la tolleranza cercata durante la soluzione del problema \`e pari a ${10}^{-6}$.\newline
    Successivamente, calcoliamo l'errore assoluto alla $k$-esima iterazione $\norm{\mathbf{e}^{(k)}}$.
    \begin{matlabcode}
...
\end{equation*}
Perfino nel terzo scenario di esecuzione non abbiamo registrato alcun incremento di velocit\`a rispetto alla versione seriale dell'algoritmo, dal momento che l'\textit{overhead} per il coordinamento e la sincronizzazione dei \textit{worker} ha largamente superato i benefici introdotti dalla parallelizzazione.

L'analisi del profilo di esecuzione di \lstinline{jacobi} mediante lo strumento \textit{Profiler} di MATLAB ha immediatamente evidenziato quale potesse essere il principale collo di bottiglia: la funzione interna \lstinline{RemoteSpmdControllerImpl.awaitBlock} ha occupato la quasi totalit\`a del tempo di esecuzione del metodo di Jacobi, indicando che i \textit{worker} sono rimasti in uno stato di attesa per un intervallo di tempo significativo e, nel frattempo, non hanno proceduto con la computazione vera e propria.

Innanzitutto, il metodo di Jacobi \`e un algoritmo fortemente sincrono; a ogni passo dell'esecuzione, impone un coordinamento tra i \textit{worker} per calcolare la nuova soluzione approssimata e per valutare le condizioni di arresto basate sul controllo del residuo relativo e sulla verifica della divergenza del metodo.\newline
L'aggiornamento del residuo $\mathbf{r}^{(k)}$, previsto da $\eqref{eq:metodoIterativoConResiduo}$, induce un'operazione di riduzione, in quanto \`e necessario recuperare l'intero vettore delle incognite $\mathbf{x}^{(k)}$ per determinare completamente il prodotto matrice-vettore $A\mathbf{x}^{(k)}$.\newline
...
Inoltre, il comportamento osservato pu\'o essere parzialmente attribuito alla latenza di comunicazione intrinseca nell'infrastruttura del Parallel Computing Toolbox; nonostante i \textit{worker} risiedano sullo stesso calcolatore, la comunicazione all'interno del \textit{parallel pool} non \`e a costo zero e il suo effetto negativo sulle prestazioni si aggiunge al ritardo accumulato in precedenza.

In conclusione, il risultato ottenuto dall'applicazione della funzione \lstinline{jacobi} alla risoluzione di un sistema di equazioni lineari di grandi dimensioni si \`e rivelato negativo in termini di prestazioni, ma ha permesso di dimostrare come la scelta dell'algoritmo influenzi le possibilit\`a di ottenere un reale \textit{speedup} delle prestazioni, mettendo in luce i limiti pratici del parallelismo a livello di dati in un sistema a memoria condivisa.
