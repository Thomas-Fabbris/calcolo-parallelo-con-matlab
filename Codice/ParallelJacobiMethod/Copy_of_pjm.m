...
arguments (Input)
    ...
    b {mustBeColumn, mustBeFloat, mustBeSameSize(A,b, 'Right-hand side')}
    tol (1,1) {mustBeFloat, mustBePositive} = PjmHelper.DefaultTol
    maxit (1,1) {mustBeInteger, mustBePositive} = PjmHelper.DefaultMaxit
    x0 {mustBeColumn, mustBeFloat, mustBeSameSize(A, x0, 'Initial guess')} = zeros(size(A,1))
end
...
function mustBeSameSize(A,v,name)
if (~isequal(size(A,1), size(v)))
